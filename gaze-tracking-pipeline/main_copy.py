...
def main(monitor_mm=None, monitor_pixels=None, model=None, visualize_preprocessing=False, visualize_laser_pointer=True, visualize_3d=False):
    # setup webcam
    source = WebcamSource(width=1280, height=720, fps=60, buffer_size=10)

    # setup monitor
    if monitor_mm is None or monitor_pixels is None:
        monitor_mm = [1920,1080]
    ...
    for frame_idx, frame in enumerate(source):
        ...
        if results.multi_face_landmarks:
            ...
            success, rvec, tvec, inliers = cv2.solvePnPRansac(face_model, face_landmarks, rvec=rvec, tvec=tvec, useExtrinsicGuess=True, flags=cv2.SOLVEPNP_EPNP)  # Initial fit
            for _ in range(10):
                success, rvec, tvec = cv2.solvePnP(face_model, face_landmarks, rvec=rvec, tvec=tvec, useExtrinsicGuess=True, flags=cv2.SOLVEPNP_ITERATIVE)  # Second fit for higher accuracy
            ...
            # data preprocessing
            face_model_transformed, face_model_all_transformed = get_face_landmarks_in_ccs(frame.shape, results, face_model, face_model_all, landmarks_ids)
            ...
            img_warped_left_eye, _, _ = normalize_single_image(image_rgb, rvec, None, left_eye_center)
            img_warped_right_eye, _, _ = normalize_single_image(image_rgb, rvec, None, right_eye_center)
            img_warped_face, _, rotation_matrix = normalize_single_image(image_rgb, rvec, None, face_center, is_eye=False)
...
if __name__ == '__main__':
    ...
    model.eval()
    main(model)
    # main(args.monitor_mm, args.monitor_pixels, model, args.visualize_preprocessing, args.visualize_laser_pointer, args.visualize_3d)
